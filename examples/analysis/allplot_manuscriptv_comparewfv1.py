...
# sys.exit("prepdone")
# %% compare dose response across waveforms
peses = []
pemeans = []
onsets_sats = {}
...
allpes = allpes.query('deformation=="Deformed"')
...
# rewrite the above with map dataframe
for diam in allpes.fiber_diam.unique():
    g = sns.FacetGrid(
        data=allpes.query(f"fiber_diam=='{diam}'"),
...
# barplot
g = sns.FacetGrid(data=scoredat, margin_titles=True, col="fiber_diam", row='deformation')
g.map_dataframe(
    sns.barplot, y="score2d3d", x="slice", hue="wf", palette=tc.tol_cset('muted'), errorbar=None, estimator='median'
)
g.map_dataframe(
    sns.stripplot,
    y="score2d3d",
...
g = sns.FacetGrid(data=scoredat, margin_titles=True, col="fiber_diam", row='deformation')
g.map_dataframe(
    sns.barplot, y="CCC", x="slice", hue="wf", palette=tc.tol_cset('muted'), errorbar=None, estimator='median'
)
g.map_dataframe(
    sns.stripplot,
    y="CCC",
