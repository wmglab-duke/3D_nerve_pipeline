...
defdefcomp = newdefdat.query('type=="true-3D" and deformation != "2D-3D"')
...
panelB = defdr.query(f"modeltype=='true-3D' and deformation!='3D-3D' and contact in {main_comparison}")
...
panelD['panel'] = 'Deformed extrusion (cathodic-leading\nslice) vs. true-3D'
...
sns.set(font_scale=1.75)
sns.set_style('whitegrid')
threshdat = threshload.query(f'contact in {main_comparison} and deformation==')
...
# %% activation order
newdefdr = defdr.copy()
newdefdr = newdefdr.query("'5R' in nerve_label and deformation=='3D-3D'").sort_values('modeltype')
...
g = sns.catplot(
    kind='swarm',
    row='fiber_diam',
    data=newdefdr.query(f"fiber_diam in [3]"),
    y='percent_activated',
    x='contact',
    units='nerve_label',
    palette='plasma',
...
g.map_dataframe(sns.boxplot, y='score2d3d', x='slice', boxprops={'facecolor': 'white'}, whis=float('Inf'))
g.map_dataframe(sns.swarmplot, y='score2d3d', x='slice', color='black', size=5)
# ax.set_xlabel('Fiber Diameter (μm)')
plt.ylabel('AR')
...
for acsrc, ax in zip(sorted(pd.unique(imdr.active_src_index)), axs):
    ...
    legend_elements = [
        Line2D([0], [0], marker='o', color='w', label='extrusion', markerfacecolor='k', markersize=10),
        Line2D([0], [0], marker='X', color='w', label='true-3D', markerfacecolor='k', markersize=10),
    ]
...
nsimdata = concats.query(
    f'fiber_diam in [3,13] and contact in {cath_comparison}'
...
axs[0].set_xlim(reversed(axs[0].get_xlim()))
axs[0].set_ylabel("Activation Location (cm)")
axs[0].set_title("extrusion-110%")
axs[1].set_title("true-3D-110%")
...
g = sns.catplot(
    kind='swarm',
    # row='deformation',
    data=newdefdr.query(f"fiber_diam in [3,13] and contact in {cath_comparison} and deformation=='3D-3D'"),
    y='percent_activated',
    x='fiber_diam',
    units='nerve_label',
    col='modeltype',
...
for deformation in ["3D-3D", "Undeformed"]:
...
sns.set(font_scale=1.75)
sns.set_style('whitegrid')
threshdat = newdefdat.query(f'contact in {cath_comparison} and deformation=="3D-3D"')
...
for comp in [an_comparison, main_comparison, cath_comparison]:
    ...
    # add correlation to plot
    # ax.text(0.02, 0.91, f'$R^2={r**2:.2f}$', transform=ax.transAxes)
    print(f'{diam} {deformation} {comp[0]} μm LCCC: {r ** 2:.2f}')
    # ax.set_title(f'{diam} μm')
    ax.plot([0, lim[deformation]], [0, lim[deformation]], '--k', linewidth=2, label='1:1 line')
    plt.legend()
    # ax.set_aspect('equal', 'box')
...
# add correlation to plot
# ax.text(0.02, 0.91, f'$R^2={r**2:.2f}$', transform=ax.transAxes)
print(f'{diam} {deformation} μm LCCC: {r ** 2:.2f}')
# ax.set_title(f'{diam} μm')
ax.plot([0, lim[deformation]], [0, lim[deformation]], '--k', linewidth=2, label='1:1 line')
# ax.set_aspect('equal', 'box')
# ax.apply_aspect()
...
g.set_ylabels('true-3D Threshold (mA)')
g.set_titles(row_template='', col_template='Deformation: {col_name}')
# %% calculate LCCC for all comparisons, deformation types
lccc_vals = []
for deformation in ["Undeformed", "3D-3D"]:
    for diam in concats.fiber_diam.unique():
        for comp in [an_comparison, main_comparison, cath_comparison]:
            lccc_dat = concats.query('contact in @comp and deformation==@deformation and fiber_diam==@diam')
            r = concordance_correlation_coefficient(lccc_dat.threshold3d, lccc_dat.threshold)
            lccc_vals.append({'deformation': deformation, 'comparison': comp[0], 'lccc': r**2, 'diam': diam})
        # also do minthresh, where comp = "combo"
        lccc_dat = minthresh.query('deformation==@deformation and fiber_diam==@diam')
        r = concordance_correlation_coefficient(lccc_dat.threshold3d, lccc_dat.threshold)
        lccc_vals.append({'deformation': deformation, 'comparison': 'combo', 'lccc': r**2, 'diam': diam})
lccc_vals = pd.DataFrame(lccc_vals)
# plot
sns.set(font_scale=1.75)
sns.set_style('white')
g = sns.relplot(
