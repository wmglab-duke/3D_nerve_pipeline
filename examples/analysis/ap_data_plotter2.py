...
g = sb.catplot(x="sample", y='activation_zpos',hue='sample',
                 col="nsim",
                data=datre, kind='strip', height = 5, aspect=.4,linewidth=0,
                order = ['2D','3D'],sharey=True)
axs = g.axes
axs[0][0].set_ylabel(u'Activation z-position (mm)')
for i,s in enumerate([2,5,8,11,13]):
    axs[0][i].set_title('fiber diam: {}\u03BCm'.format(s))
plt.subplots_adjust(top=.88)
plt.suptitle("Activation z-position for sample {} (2D model: slice at {} contact)".format(sample_name,bigcomp[1]),fontsize=15)
for ax in axs[0]:
    ln = ax.hlines([cuff_contacts],.4,.6,color = 'red')
