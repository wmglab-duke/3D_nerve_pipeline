...
for d, n in zip(diams, n_sims):
    ...
    plt.figure()
    plt.plot(coords2[:, -1], ve2, label='twood')
    plt.plot(coords3[:, -1], ve3, label='threed')
    plt.legend()
    plt.title(f'Ve for {d} micron fibers')

    # plot first differential
    plt.figure()
    plt.plot(coords2[:, -1][1:], np.diff(ve2), label='twood')
    plt.plot(coords3[:, -1][1:], np.diff(ve3), label='threed')
    plt.title(f'First differential of Ve for {d} micron fibers')
    plt.legend()

    # plot second differential
    plt.figure()
    plt.plot(coords2[:, -1][1:-1], np.diff(np.diff(ve2)), label='twood')
    plt.plot(coords3[:, -1][1:-1], np.diff(np.diff(ve3)), label='threed')
    plt.title(f'Second differential of Ve for {d} micron fibers')
    plt.legend()
    ve2 = ve2
