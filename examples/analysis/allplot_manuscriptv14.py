...
repeated = matched.melt(
    ...
    var_name='type',
    value_name='thresholdmatched',
).rename({'thresholdmatched': 'threshold'})
...
alldr['deformation'] = pd.Categorical(
    alldr['deformation'], ordered=True, categories=['Undeformed', 'Structural', 'ASCENT']
)
...
for comparison in [an_comparison, center_comparison, cath_comparison]:
    ...
    compiled_data = analysis.melt(
        id_vars=['sample', 'fiber_diam', 'sim', 'type', 'nerve_label', 'model', 'nsim', 'deformation'],
...
compmatch = datamatch_merge(
    ...
    'threshold',
    merge_cols=["model", "sim", "nerve_label", "nsim", "level", "deformation", 'contact'],
).drop(columns='type')
...
plt.figure()
sns.scatterplot(data=thismatch, x='threshold', y='threshold3d', hue='inner', palette='rainbow', legend=False)
plt.xlim(0, 4)
plt.ylim(0, 4)
...
for nerve in pd.unique(threshdat['nerve_label']):
    for deformtype in ['Undeformed', 'Structural']:
        for contact in ['3D', 'cathodic', 'center', 'anodic']:
            ...
            scores.append({'sample': nerve, 'scoresmolbeeg': rc, 'deformation': deformtype, 'slice': contact})
fig = plt.figure()
scoredat = pd.DataFrame(scores)
sns.stripplot(
    data=scoredat,
...
# barplot
plt.xticks([0, 1, 2, 3], ['true-3D', 'extrusion\n(cathodic)', 'extrusion\n(center)', 'extrusion\n(anodic)'])
plt.xlabel('model')
# %% Percent Error deformed
sns.reset_orig()
...
imdr['active_src_index'] = pd.Categorical(
    imdata['active_src_index'], categories=sorted(imdata['active_src_index'].unique()), ordered=True
)
...
for nerve_label in pd.unique(newim.nerve_label):
    ...
    sm.set_array([])
    plt.gcf().colorbar(
        sm, ax=plt.gca(), aspect=10, shrink=0.8, label='Proportion true-3D\nfibers activated', pad=0.2
    ).ax.yaxis.set_ticks_position('left')
    plt.xlabel('Active contact')
...
for cc in sorted(imdata['active_src_index'].unique()):
    ...
    for nerve_label in pd.unique(shortdat['nerve_label']):
        for fiber_diam in shortdat.fiber_diam.unique():
            ...
            rc = compute_reorder_cost(list(data2d), list(data3d))
            scores.append(
                {'active_src_index': cc, 'score2d3d': rc, 'nerve_label': nerve_label, 'fiber_diam': fiber_diam}
            )
plt.figure()
scoredat = pd.DataFrame(scores)
g = sns.lineplot(
    data=scoredat, y='score2d3d', x='active_src_index', hue='nerve_label', palette=defpal, style='fiber_diam'
)
sns.move_legend(g, [1.05, 0])
plt.xlabel('Active Contact')
plt.ylabel('Activation Reordering')
plt.figure()
g = sns.barplot(data=scoredat, y='score2d3d', x='active_src_index', hue='fiber_diam', palette='binary')
plt.xlabel('Active Contact')
plt.ylabel('Activation Reordering')
...
def recruitment_cost(data, activated=1):
    """Calculate recruitment cost for each inner.
    :param activated: proportion of on-target fibers activated.
...
# rewrite the above with map dataframe
g = sns.FacetGrid(
    data=alldfcomp, palette=defpal, row='level', row_order=['saturation', 'half', 'onset'], margin_titles=True
)
g.map_dataframe(
    sns.lineplot,
    x='active_src_index',
...
for nerve_label, samplenum, r_cuff_in_pre_MCT in zip(
    ...
    handles = [
        plt.Line2D(
            [0], [0], marker='o', color='w', label='on target', markerfacecolor='orange', markersize=10, linewidth=0
        ),
        plt.Line2D(
            [0], [0], marker='o', color='w', label='off target', markerfacecolor='blue', markersize=10, linewidth=0
        ),
    ]
            ...
            for t in pd.unique(imdata['type']):
                for percent_ontarget in [0.25, 0.5, 0.75, 1]:
                    ...
                    imdatain['topo'] = imdatain.apply(
                        lambda row: 'on_target' if row['inner'] in ontarget else 'off_target', axis=1
                    )
    ...
    plt.figure()
    imdatplot = imdatfasr.query('fiber_diam in [3] and topo=="on_target"')
    g = sns.stripplot(
    ...
    legend_elements = [
        ...
        Line2D([0], [0], linestyle='-', color=plasma(0), label='25% on-target', linewidth=2),
        Line2D([0], [0], linestyle='-', color=plasma(1), label='50% on-target', linewidth=2),
        Line2D([0], [0], linestyle='-', color=plasma(2), label='75% on-target', linewidth=2),
        Line2D([0], [0], linestyle='-', color=plasma(3), label='100% on-target', linewidth=2),
    ]
    legend_labels = ['extrusion', 'true-3D', '25% on-target', '50% on-target', '75% on-target', '100% on-target']
    plt.legend(handles=legend_elements, labels=legend_labels)
    plt.ylabel('off-target activated (%)')
    plt.ylim(-0.1, 1.1)
    plt.xlabel('Active contact')
    plt.xticks(range(5), list(range(4)) + ['LN'])
    plt.title(f'Nerve: {nerve_label} - D: 3 μm')
    plt.figure()
