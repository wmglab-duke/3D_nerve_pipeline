...
for sample in samples:
    for model in models:
        for sim in sims:
            simpath = f'samples/{sample}/models/{model}/sims/{sim}'
            for folder in os.listdir(simpath + '/n_sims'):
                inpath = f'{simpath}/n_sims/{folder}/data/outputs/AP_info.csv'
                dat2d.append(pd.read_csv(inpath))
for model in models:
    for sim in sims:
        simpath = f'samples/{samp3d}/models/{model}/sims/{sim}'
        for folder in os.listdir(simpath + '/n_sims'):
            inpath = f'{simpath}/n_sims/{folder}/data/outputs/AP_info.csv'
...
for sample in samples:
    ...
    plot = ggpubr.ggpaired(
        plotdata,
        cond1="activation_zpos",
        cond2="activation_zpos3d",
        color="condition",
        line_color="gray",
        line_size=0.5,
        point_size=1.2,
        ...
        legend="none",
        scales="free_y",
        title=f"Activation position for 2D ex models vs. full-3D model {samp3d}vs{sample}",
    )
