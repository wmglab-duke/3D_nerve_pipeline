...
def pt_src(sourcepoint, nodepoint, current=1, rho=1):
    ...
    dist = np.sqrt(
        (sourcepoint[0] - nodepoint[0]) ** 2
        + (sourcepoint[1] - nodepoint[1]) ** 2
        + (sourcepoint[2] - nodepoint[2]) ** 2
    )
    # calculate potential
...
for sample, samp3d, nerve_label in zip(
    ...
    for i, row in threshdat.iterrows():
        ...
        fiber3d = os.path.join(
            'samples', str(samp3d), 'models', str(model), 'sims', str(sim), 'fibersets', str(0), f'{fiber3d}.dat'
        )
