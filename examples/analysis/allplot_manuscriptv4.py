...
for i, row in threshload.iterrows():
    if row['type'] == '3D':
        ...
        data = quickinner.query(
            f"nerve_label == '{nerver}' and master_fiber_index == @row.master_fiber_index and type == '2D'"
        )['inner'].values
        threshload.loc[i, 'inner'] = data[0]
...
#%% threshold coeff intrafascicle and inter
sns.set(font_scale=1.75, style='whitegrid')  # TODO remove anodic
vardat = repeated.query('nsim in [0,5] and sim==3 and contact=="cathodic"')
vardat = vardat[vardat['contact'] != "anodic"]
grouped = vardat.groupby(['EMsample', 'fiber_diam', 'sim', 'type', 'inner'])
...
for comparison in [
    ...
    # g.map_dataframe(sns.stripplot, hue='Sample', y='correlation',dodge=True, palette='colorblind')
    # g.map_dataframe(sns.boxplot,y='correlation', boxprops={'facecolor':'None'},whis=100)
    sns.stripplot(data=corrs, palette='RdPu', y='contact', hue='fiber_diam', x='correlation', dodge=True)
...
plt.figure()
sns.lineplot(
    data=pes, style='level', y='pe', x='fiber_diam', markers=True, estimator=None, units='nerve_label', hue='level'
)
plt.xlabel('Fiber Diameter (μm)')
plt.ylabel('Percent Error')
...
sns.set_style('whitegrid')
plt.figure()
sns.lineplot(data=pemean, style='level', y='pe', x='fiber_diam', markers=True, hue='level')
plt.xlabel('Fiber Diameter (μm)')
plt.ylabel('Percent Error')
