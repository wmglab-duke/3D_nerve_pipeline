...
sns.set(font_scale=1.75)
sns.set_style('whitegrid')
# remove all rows with nerve label containing "def"
threshes = pd.read_csv('thresh_unmatched_sim3.csv').query('sim==3')
# replace sample 6731 with 6739
threshes['sample'] = threshes['sample'].replace({6731: 6739, 2531: 2539, 3731: 3739, 5731: 5739})
# remove all rows where nerve label contains def and type is 2D
newdefdat = threshes[~((threshes['nerve_label'].str.contains('def')) & (threshes['type'] == '2D'))]
# replace all 'def' with 'asc' in nerve labels
newdefdat['nerve_label'] = newdefdat['nerve_label'].str.replace('def', 'asc')
# add new column "deformed" which is true if nerve label contains "asc"
newdefdat['deformed'] = newdefdat['nerve_label'].str.contains('asc')
# find mean threshold for each inner
# agged = newdefdat.groupby(['sample','inner','deformed','peri_thk'])['threshold'].mean().reset_index()
# generate boxplot of 2DEM and 3DM thresholds
# remove asc from all nerve labels
newdefdat['nerve_label'] = newdefdat['nerve_label'].str.replace('asc', '')
...
# remove all rows where samples is a 3 digit int and deformed is true
newdefdat = newdefdat[~((newdefdat['sample'].astype(str).str.len() == 3) & (newdefdat['deformed'] == True))]
#%%
g = sns.catplot(
    kind='violin',
    col='deformed',
...
new2dem = (
    ...
    .groupby(['sample', 'nerve_label', 'peri_thk', 'fiber_diam', 'deformed', 'inner'])['threshold']
    .mean()
    .reset_index()
)
...
allmatch = pd.merge(newmatch, new2dem, on=['nerve_label', 'fiber_diam', 'deformed', 'inner'])
...
for diam, pos, defstatus, ax in zip([3, 3, 13, 13], (0.2, 0.8, 0.2, 0.8), (False, True, False, True), g.axes.ravel()):
    # ax.set_title(f'{diam}, {defstatus}')
    limmax = max([ax.get_xlim()[1], ax.get_ylim()[1]])
    rdata = nsimdata.query(f'fiber_diam=={diam} and deformed == {defstatus}')
...
grouped = vardat.groupby(['sample', 'fiber_diam', 'sim', 'type', 'inner', 'deformed'])
analysis = grouped.agg({'threshold': [np.var, np.mean]})
analysis.columns = ["_".join(col_name).rstrip('_') for col_name in analysis.columns]
analysis.reset_index(inplace=True)
...
# now do variance between fascicle mean thresholds
grouped = analysis.groupby(['sample', 'fiber_diam', 'type', 'deformed'])
analysis = grouped.agg({'threshold_mean': [np.var]})
analysis.columns = ["_".join(col_name).rstrip('_') for col_name in analysis.columns]
analysis.reset_index(inplace=True)
...
sns.set(font_scale=2)
sns.set_style('whitegrid')
nsimdata = newdefdat.query('fiber_diam in [3,13]')
actlocdata = nsimdata.query('type=="3DM"')
actlocdata['type'] = '3DM*'
# actlocdata['peri_thk'] = actlocdata['peri_thk_act_site']
nsimdata = pd.concat([nsimdata, actlocdata])
