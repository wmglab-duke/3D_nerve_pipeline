    ...
    for i, s in enumerate([2, 5, 8, 11, 13]):
        for label_ind, sample in enumerate(samples):
            ...
            compdat_pandas.append(
                {'Sample': sampname, '2D slice': act_labels[label_ind], 'fiber diam': s, 'var2': var2, 'case': '2D'}
            )
            compdat_pandas.append(
                {'Sample': sampname, '2D slice': act_labels[label_ind], 'fiber diam': s, 'var2': var3, 'case': '3D'}
            )
#%%
plt.figure()
...
g = sns.swarmplot(
    data=allcorr, x='fiber diam', y='correlation', style='2D slice', hue='Sample', s=100, palette='colorblind'
)
g = sns.lineplot(
    data=allcorr, x='fiber diam', y='correlation', style='2D slice', hue='Sample', legend=False, palette='colorblind'
)
plt.legend(bbox_to_anchor=(1.53, 0.5), loc='center right', borderaxespad=0)
