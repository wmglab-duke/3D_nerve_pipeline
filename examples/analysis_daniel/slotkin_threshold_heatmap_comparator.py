...
for samp2d, samp3d, pairname, slidename in zip(samp2ds, samp3ds, pairnames, slidenames):
    ...
    fiberpoints = np.array([x['fiber'][0][:2] for x in sim_obj.fibersets[0].fibers])
    fiberpoints[:, 1] += -np.amin(fiberpoints[:, 1])
    fiberpoints[:, 0] += -np.amin(fiberpoints[:, 0])
    # threshdat.threshold*=256/threshdat.threshold.max()
    weights2d = threshdat.query('fiber_diam==3 and sample==@samp2d').sort_values('master_fiber_index').threshold
    weights3d = threshdat.query('fiber_diam==3 and sample==@samp3d').sort_values('master_fiber_index').threshold
    fiber2d = np.concatenate([np.array(weights2d)[:, None], fiberpoints], axis=1)
    fiber3d = np.concatenate([np.array(weights3d)[:, None], fiberpoints], axis=1)
    ...
    ax1.imshow(sig_to_img(fiber2d), vmax=np.amax(fiber2d), vmin=np.amin(fiber2d), cmap='gray')
    ...
    sns.histplot(fiber2d[:, 0], label="2DEM", fill=False, color='blue')
