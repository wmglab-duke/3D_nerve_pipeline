...
for i in range(0, len(allsum), 2):
    ax[0].plot(np.arange(-lr * dz, lr * dz, dz), allsum[i], 'k', label='split' if i == 0 else '_')
    ax[0].plot(np.arange(-lr * dz, lr * dz, dz), alldiff[i], 'r', label='merge' if i == 0 else '_')
    plt.xlabel('z position (microns)')
    plt.ylabel('Second difference of Ve')
    ax[0].axvline(x=0, color='k', linestyle='--')
    ax[0].set_title(f'approach, dz={dz}')
    plt.legend()
    ax[1].plot(np.arange(-lr * dz, lr * dz, dz), allsum[i + 1], 'k', label='split' if i == 0 else '_')
    ax[1].plot(np.arange(-lr * dz, lr * dz, dz), alldiff[i + 1], 'r', label='merge' if i == 0 else '_')
    plt.xlabel('z position (microns)')
    plt.ylabel('Second difference of Ve')
    plt.axvline(x=0, color='k', linestyle='--')
    ax[1].set_title(f'recede, dz={dz}')
    plt.legend()
