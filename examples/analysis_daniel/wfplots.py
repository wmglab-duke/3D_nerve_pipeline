...
fig, axs = plt.subplots(3, 2, figsize=(12, 6), sharey=True, sharex=True)  # Two columns
for ax in axs.ravel():
...
axs[0, 0].plot(t_wave1_adj * 1e6, -wave1_adj, label="Asymmetric & Bipolar", lw=2, color=colors[0])
axs[1, 0].plot(t_wave1_adj * 1e6, -wave1_adj, label="Asymmetric & Monopolar", lw=2, color=colors[2], linestyle='-')

axs[2, 0].plot(t_wave2_adj * 1e6, -wave2_adj, label="Symmetric & Bipolar", linestyle='-', lw=2, color=colors[1])

# now plot "Asymmetric monopolar", which is identical to the asymmetric biphasic waveform
axs[0, 0].set_title("cathodic contact")
axs[1, 0].set_xlabel("Time (μs)")
axs[2, 0].set_ylabel("Unscaled amplitude")
# Plot polarity-flipped waveforms
axs[0, 1].plot(t_wave1_adj * 1e6, wave1_adj, label="Flipped Asymmetric Bipolar", lw=2, color=colors[0])
...
axs[2, 1].plot(t_wave2_adj * 1e6, wave2_adj, label="Flipped Symmetric Bipolar", linestyle='-', lw=2, color=colors[1])

axs[0, 1].set_title("anodic contact")
