...
plotactidata['difference'] = plotactidata['percent_activated'] - plotactidata['percent_activated3d']
...
# Add stems to the plot
for ax in g.axes.ravel():
    data = plotactidata.query(f"contact == '{ax.get_title().split('=')[1].strip()}'")
    for _, row in data.iterrows():
        ax.plot(
            [row['percent_activated'], row['percent_activated']],
            [0, row['difference']],
            color='gray',
            linestyle='-',
            linewidth=0.5,
...
# Set the titles and labels
g.set_titles(row_template="{row_name}", col_template="{col_name}")
g.set_xlabels("Prop. fibers active (3 μm)")
g.set_ylabels("Difference (Extrusion - True-3D)")
...
g.set_xlabels("Prop. fibers active (13 μm)")
g.set_ylabels("Prop. fibers active (13 μm)")
...
# legend.set_title("D (μm)")
plt.gcf().set_size_inches(3, 2.5)
print(deffinalmatch.groupby(["nerve_label", "fiber_diam"]).agg(np.median)['pe'].max())
