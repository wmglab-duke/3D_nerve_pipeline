...
g = sns.FacetGrid(data=scoredat, margin_titles=True, col="deformation")
g.map_dataframe(
    sns.barplot,
    y="CCC",
    x="slice",
...
# %% activation order fiberdiam
newdefdr = defdr.copy()
newdefdr = newdefdr.query("'2L' in nerve_label and deformation!='ASCENT'").sort_values("modeltype")
newdefdr["deformation"] = pd.Categorical(newdefdr["deformation"], categories=["Undeformed", "Structural"])
sns.set(font_scale=1.5)
sns.set_style("whitegrid")
newdefdr["percent_activated3"] = np.nan
...
for i, mod in enumerate(newdefdr.modeltype.sort_values().unique()):
    ...
    data2d = shortdat["percent_activated"]
    data3d = shortdat["percent_activated3"]
    rc = concordance_correlation_coefficient(list(data2d), list(data3d))
    ax = g.axes[0][i]
