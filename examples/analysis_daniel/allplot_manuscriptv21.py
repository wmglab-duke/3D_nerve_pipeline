...
sns.move_legend(g, [0.73, 0.2], facecolor='white', framealpha=1, frameon=True, edgecolor='black')
...
g = sns.FacetGrid(
    ...
    row_order=['Undeformed', 'ASCENT', 'Structural'],
    margin_titles=True,
)
g.map_dataframe(
    sns.swarmplot,
...
g.map_dataframe(
    ...
    linewidth=0.5,
    edgecolor='w',
    order=['Undeformed', 'ASCENT', 'Structural'],
)
g.map_dataframe(
    sns.pointplot,
    x="deformation",
...
threeddefmatch = deftomatch.query(f'deformation=="Structural" and contact in {cath_comparison}')
...
for nerve_label in ['2L', '3R', '5R', '6R']:
    newdefdr = defdr.copy()
    newdefdr = newdefdr.query(f"'{nerve_label}' in nerve_label and deformation!='ASCENT'").sort_values("modeltype")
    newdefdr["deformation"] = pd.Categorical(newdefdr["deformation"], categories=["Undeformed", "Structural"])
    newdefdr = newdefdr.query(f'deformation=="Structural" and contact in {cath_comparison}')
...
nsimdata = concats.query("fiber_diam in [3] and deformation=='Structural'")  # TODO replace all cath comparison with non
g = sns.relplot(
...
for comparison in [cath_comparison]:  # for stringdat in ["Undeformed", "Structural", "ASCENT"]:
    for stringdat in ["Structural", "ASCENT"]:
        thiscontact = comparison[0]
        subdat = newdefdat.query(f"deformation=='{stringdat}' and contact in {comparison}")
        levels = {
            "onset": 10,
