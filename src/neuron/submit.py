...
def make_fiber_tasks(submission_list, submission_context):
    ...
    for sim_name, runfibers in submission_list.items():
        ...
        for fiber_data in runfibers: #TODO cuff type seems pointless here.. would always be src or empty for potentials_path...
            ...
            if cuff_type:
                potentials_path = os.path.join(sim_path, 'data', 'inputs', f'{cuff_type[0]}_inner{inner_ind}_fiber{fiber_ind}.dat')
        ...
        for model in models:
            for sim in sims:
                ...
                for sim_name in nsim_list:
                    ...
                    fibers_files = [x for x in os.listdir(fibers_path) if re.match('(?:(src)_)?inner[0-9]+_fiber[0-9]+\\.dat', x)]  # First regex group with ? is optional - for backwards compatibility
