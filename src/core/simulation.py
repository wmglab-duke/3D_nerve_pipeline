...
def get_z_coords(root, file):
    ...
    with open(os.path.join(root, file)) as coords_file:
        ...
        for coords_file_line in coords_file_lines:
            coords = np.append(
                coords,
                float(coords_file_line.split(' ')[2]),  # this is the z coord #TODO Check if this is broken in master
            )
    return coords
...
class Simulation(Configurable, Saveable):
    ...
    def build_n_sims(self, sim_dir, sim_num, threed=False) -> 'Simulation':
    ...
    def activations_exist(
        sample: int, model: int, sim: int, sim_dir: str, source: str, n_amps: int, verbose: bool = True
    ):
