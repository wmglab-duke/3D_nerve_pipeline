...
class Sample(Exceptionable, Configurable, Saveable):
    ...
    def im_preprocess(self,path):
        ...
        if self.search(Config.SAMPLE, 'image_preprocessing')['fill_holes']:
            img = binary_fill_holes(img)
        removal_size = self.search(Config.SAMPLE, 'image_preprocessing')['object_removal_area']
        if self.search(Config.SAMPLE, 'image_preprocessing')['object_removal_units']=="MICRONS":
            removal_size = removal_size/self.factor/self.factor
        elif self.search(Config.SAMPLE, 'image_preprocessing')['object_removal_units']!="PIXELS":
            self.throw(110)
